...
d = read.csv('Wichita.csv') #Load in data
...
df2 = subset(d2, select = c(Date,Wheat)) #Create dataframe with relevant data from price dataframe
...
for (i in 1:15){
  model = grangertest(p$Wheat~p$AVPRCP, order = i)
  #print(i)
  #print(model)
...
plot(month_order,p_value,ylab = 'p value', xlab = 'Months of lag', main = 'Causality Wheat price based on precipitation in Wichita')
