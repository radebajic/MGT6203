...
df2 = subset(d2, select = c(Date,Natural.Gas...Spot.price.at.the.Henry.Hub.terminal.in.Louisiana)) #Create dataframe with relevant data from price dataframe
...
for (i in 1:15){
model = grangertest(p$Natural.Gas...Spot.price.at.the.Henry.Hub.terminal.in.Louisiana~p$average_temp, order = i)
print(i)
print(model)
...
plot(month_order,p_value,ylab = 'p value', xlab = 'Months of lag', main = 'Causality gas price based on US temperature')
